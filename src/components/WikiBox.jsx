...
export default function WikiBox() {
    ...
    return await fetch(
      ...
      .then((data) =>
        data !== undefined
          ? setWikipedia(data)
          : () => {
              console.warn('No Wikipedia found for ' + city);
              setWikipedia(['No Wikipedia article exists for ' + city]);
            }
      );
  }
  ...
  async function reverseGeo(latitude, longitude) {
    let response = await fetch(
      `https://maps.googleapis.com/maps/api/geocode/json?latlng=${latitude},${longitude}&key=AIzaSyCZXol-ZruQJH-gc_eqlf2RAR4H7VRtaIQ`
    );
